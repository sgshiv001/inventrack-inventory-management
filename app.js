...
function initEarthCanvas(){
  ...
  const vertexShaderSource=`attribute vec3 aPosition;attribute vec3 aNormal;attribute vec2 aUv;uniform vec2 uCanvas;uniform float uRadius;uniform float uYaw;uniform float uPitch;uniform float uZoom;varying vec2 vUv;varying vec3 vNormal;varying vec3 vPosition;vec3 rotateY(vec3 p,float a){float c=cos(a),s=sin(a);return vec3(c*p.x+s*p.z,p.y,-s*p.x+c*p.z);}vec3 rotateX(vec3 p,float a){float c=cos(a),s=sin(a);return vec3(p.x,c*p.y-s*p.z,s*p.y+c*p.z);}void main(){vec3 p=rotateX(rotateY(aPosition,uYaw),uPitch);vUv=aUv;vNormal=rotateX(rotateY(aNormal,uYaw),uPitch);vPosition=p;vec2 pixel=vec2(300.0,198.0)+p.xy*uRadius*uZoom;vec2 clip=vec2(pixel.x/uCanvas.x*2.0-1.0,1.0-pixel.y/uCanvas.y*2.0);gl_Position=vec4(clip,p.z*0.5,1.0);}`;
  const fragmentShaderSource=`precision mediump float;uniform sampler2D uTexture;varying vec2 vUv;varying vec3 vNormal;varying vec3 vPosition;void main(){vec3 normal=normalize(vNormal);vec3 light=normalize(vec3(-0.45,0.52,1.0));float diffuse=max(dot(normal,light),0.0);float rim=pow(1.0-max(dot(normal,vec3(0.0,0.0,1.0)),0.0),2.2);vec3 albedo=texture2D(uTexture,vUv).rgb;vec3 color=albedo*(0.34+diffuse*0.86)+vec3(0.02,0.12,0.2)*rim;gl_FragColor=vec4(color,1.0);}`;
  ...
  const positions=[],normals=[],uvs=[],indices=[],longitudeSegments=96,latitudeSegments=48;
  for(let y=0;y<=latitudeSegments;y++){const latitude=-Math.PI/2+y/latitudeSegments*Math.PI,cosLatitude=Math.cos(latitude),sinLatitude=Math.sin(latitude);for(let x=0;x<=longitudeSegments;x++){const longitude=-Math.PI+x/longitudeSegments*Math.PI*2,cosLongitude=Math.cos(longitude),sinLongitude=Math.sin(longitude);positions.push(cosLatitude*cosLongitude,sinLatitude,cosLatitude*sinLongitude);normals.push(cosLatitude*cosLongitude,sinLatitude,cosLatitude*sinLongitude);uvs.push(1-x/longitudeSegments,1-(y/latitudeSegments));}}
  ...
  const buffer=(target,data,size,type)=>{const handle=gl.createBuffer();gl.bindBuffer(target,handle);gl.bufferData(target,data,type||gl.STATIC_DRAW);return handle};
  ...
  const image=new Image();image.onload=()=>{gl.bindTexture(gl.TEXTURE_2D,texture);gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL,true);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,image);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR)};image.src='assets/earth-texture.png';
  let dragging=false,lastX=0,lastY=0;
  const draw=()=>{gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.enable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.useProgram(program);gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,texture);gl.uniform2f(canvasUniform,canvas.width,canvas.height);gl.uniform1f(radiusUniform,166);gl.uniform1f(yawUniform,(55+globeRotation-90)*Math.PI/180);gl.uniform1f(pitchUniform,(20+globeTilt)*Math.PI/180);gl.uniform1f(zoomUniform,globeZoom);gl.drawElements(gl.TRIANGLES,indices.length,gl.UNSIGNED_SHORT,0);globeAnimationFrame=requestAnimationFrame(draw)};
  ...
  canvas.onpointermove=event=>{if(!dragging)return;globeRotation+=(event.clientX-lastX)*.32;globeTilt=Math.max(-38,Math.min(38,globeTilt-(event.clientY-lastY)*.24));lastX=event.clientX;lastY=event.clientY};
  canvas.onpointerup=event=>{dragging=false;canvas.releasePointerCapture?.(event.pointerId);canvas.classList.remove('is-dragging');renderDistributionNetwork()};
  canvas.onpointercancel=()=>{dragging=false;canvas.classList.remove('is-dragging');renderDistributionNetwork()};
  canvas.onwheel=event=>{event.preventDefault();globeZoom=Math.max(.78,Math.min(1.32,globeZoom-(event.deltaY>0?.06:-.06)));renderDistributionNetwork()};
  $('globeReset')?.addEventListener('click',()=>{globeRotation=0;globeTilt=0;globeZoom=1;renderDistributionNetwork()});
  $('globeZoomIn')?.addEventListener('click',()=>{globeZoom=Math.min(1.32,globeZoom+.1);renderDistributionNetwork()});
  $('globeZoomOut')?.addEventListener('click',()=>{globeZoom=Math.max(.78,globeZoom-.1);renderDistributionNetwork()});
  draw();
}
function renderDistributionNetwork(){
  ...
  const radians=Math.PI/180,cx=300,cy=198,radius=166*globeZoom,centerLongitude=55+globeRotation,centerLatitude=(20+globeTilt)*radians;
  const point=place=>{const longitude=(place.longitude-centerLongitude)*radians,latitude=place.latitude*radians,visibility=Math.sin(centerLatitude)*Math.sin(latitude)+Math.cos(centerLatitude)*Math.cos(latitude)*Math.cos(longitude);return {x:cx+radius*Math.cos(latitude)*Math.sin(longitude),y:cy-radius*(Math.cos(centerLatitude)*Math.sin(latitude)-Math.sin(centerLatitude)*Math.cos(latitude)*Math.cos(longitude)),visible:visibility>-0.08}};
  const pathFor=(coordinates,close=false)=>{let path='',drawing=false;for(const [longitude,latitude] of coordinates){const p=point({longitude,latitude});if(!p.visible){drawing=false;continue}path+=`${drawing?'L':'M'}${p.x.toFixed(1)} ${p.y.toFixed(1)} `;drawing=true}return path+(close&&drawing?'Z':'')};
  const graticule=[];for(let longitude=-120;longitude<=170;longitude+=20)graticule.push(pathFor(Array.from({length:71},(_,index)=>[longitude,-70+index*2])));for(let latitude=-60;latitude<=70;latitude+=20)graticule.push(pathFor(Array.from({length:146},(_,index)=>[-125+index*2,latitude])));
  const countryShapes=[
  ...
  const countryBoundaries=[[[68,24],[73,27],[79,26],[83,30],[88,27]],[[68,18],[74,16],[79,12],[84,10]],[[50,30],[55,27],[60,24],[66,23]],[[95,17],[104,14],[113,14],[122,18]],[[107,2],[112,-4],[118,-8],[124,-9]],[[4,51],[10,48],[16,45],[23,43]],[[25,35],[30,31],[35,28],[40,25]],[[138,35],[142,30],[146,24]]].map(line=>`<path d="${pathFor(line)}"/>`).join('');
  const hub=regions.find(region=>region.city==='Mumbai')||regions[0],hubPoint=point(hub),selected=regions.find(region=>region.id===selectedRegionId)||regions[0],selectedPoint=point(selected),totalSales=regions.reduce((sum,region)=>sum+region.sales,0),totalUnits=regions.reduce((sum,region)=>sum+region.units,0),shipments=Array.isArray(db.shipments)?db.shipments:[];
  const curvePath=(from,to)=>{const dx=to.x-from.x,dy=to.y-from.y,length=Math.hypot(dx,dy)||1,curve=Math.min(58,length*.27),controlX=(from.x+to.x)/2+dy/length*curve,controlY=(from.y+to.y)/2-dx/length*curve;return `M${from.x.toFixed(1)} ${from.y.toFixed(1)} Q${controlX.toFixed(1)} ${controlY.toFixed(1)} ${to.x.toFixed(1)} ${to.y.toFixed(1)}`};
  ...
  const shipmentRoutes=shipments.map(shipment=>{const from=point(shipment.origin),to=point(shipment.destination);if(!from.visible||!to.visible)return '';return `<path class="route shipment-route ${shipment.status}" d="${curvePath(from,to)}" marker-end="url(#arrow-${shipment.status==='delayed'?'risk':shipment.status==='delivered'?'healthy':'watch'})"><title>${escapeHtml(shipment.tracking)} · ${escapeHtml(shipment.origin.label)} → ${escapeHtml(shipment.destination.label)}</title></path>`}).join('');
  const countryLabels=[['India',22,79],['UAE',24,54],['Singapore',1.35,103.8],['United Kingdom',54,-2],['Kenya',0,37],['Australia',-25,134],['Japan',36,138]];
  const labels=countryLabels.map(([name,latitude,longitude])=>{const p=point({latitude,longitude});return p.visible?`<g class="country-label"><circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="1.4"/><text x="${(p.x+4).toFixed(1)}" y="${(p.y+3).toFixed(1)}">${name}</text></g>`:''}).join('');
  ...
  $('globeStage').innerHTML=`<canvas id="earthCanvas" class="earth-canvas" width="600" height="400" role="img" aria-label="Interactive 3D Earth globe. Drag to rotate and tilt, scroll to zoom."></canvas><svg class="globe-overlay" viewBox="0 0 600 400" aria-hidden="true"><defs><clipPath id="globeClip"><circle cx="${cx}" cy="${cy}" r="${radius}"/></clipPath><marker id="arrow-healthy" viewBox="0 0 6 6" refX="5" refY="3" markerWidth="4" markerHeight="4" orient="auto"><path d="M0 0L6 3L0 6Z" fill="#61e3c2"/></marker><marker id="arrow-watch" viewBox="0 0 6 6" refX="5" refY="3" markerWidth="4" markerHeight="4" orient="auto"><path d="M0 0L6 3L0 6Z" fill="#f5bd62"/></marker><marker id="arrow-risk" viewBox="0 0 6 6" refX="5" refY="3" markerWidth="4" markerHeight="4" orient="auto"><path d="M0 0L6 3L0 6Z" fill="#ff7979"/></marker></defs><ellipse class="globe-shadow" cx="300" cy="374" rx="130" ry="12"/><g clip-path="url(#globeClip)"><g class="globe-grid">${graticule.map(path=>`<path d="${path}"/>`).join('')}</g><g class="country-boundaries">${countryBoundaries}</g>${territoryRoutes}${shipmentRoutes}</g><circle class="globe-sphere-outline" cx="${cx}" cy="${cy}" r="${radius}"/>${labels}${nodes}<g class="hub-marker"><circle cx="${hubPoint.x.toFixed(1)}" cy="${hubPoint.y.toFixed(1)}" r="9"/><text x="${(hubPoint.x+12).toFixed(1)}" y="${(hubPoint.y+20).toFixed(1)}">PRIMARY HUB</text></g>${selectedLabel}<g class="globe-legend"><circle class="healthy" cx="182" cy="386" r="3"/><text x="190" y="389">Territory</text><circle class="watch" cx="250" cy="386" r="3"/><text x="258" y="389">In transit</text><circle class="risk" cx="336" cy="386" r="3"/><text x="344" y="389">At risk</text></g></svg><div class="globe-controls" aria-label="Globe controls"><button id="globeZoomOut" class="globe-control" type="button" title="Zoom out">−</button><button id="globeReset" class="globe-control globe-reset" type="button">Reset</button><button id="globeZoomIn" class="globe-control" type="button" title="Zoom in">+</button><span>Drag to rotate · scroll to zoom</span></div>`;
  initEarthCanvas();
  const insight=region=>({growth:region.id==='r1'?18:region.id==='r2'?14:region.id==='r3'?9:region.id==='r4'?22:region.id==='r5'?16:7,orders:region.id==='r1'?42:region.id==='r2'?36:region.id==='r3'?27:region.id==='r4'?19:region.id==='r5'?15:11});
