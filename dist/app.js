...
let globeAnimationFrame=0,globeRotation=0;
...
function initEarthCanvas(){
  ...
  const ctx=canvas.getContext('2d'),texture=new Image();texture.src='assets/earth-texture.png';
  let dragging=false,lastX=0;
  const drawTexture=(image,sourceX,sourceY,sourceWidth,sourceHeight,destX,destY,destWidth,destHeight)=>{
    const width=image.width;if(sourceX+sourceWidth<=width){ctx.drawImage(image,sourceX,sourceY,sourceWidth,sourceHeight,destX,destY,destWidth,destHeight);return}
    const first=width-sourceX,firstWidth=destWidth*(first/sourceWidth);ctx.drawImage(image,sourceX,sourceY,first,sourceHeight,destX,destY,firstWidth,destHeight);ctx.drawImage(image,0,sourceY,sourceWidth-first,sourceHeight,destX+firstWidth,destY,destWidth-firstWidth,destHeight);
  };
  const paint=()=>{
    const width=canvas.width,height=canvas.height,cx=300,cy=198,radius=166;
    ctx.clearRect(0,0,width,height);ctx.save();ctx.beginPath();ctx.arc(cx,cy,radius,0,Math.PI*2);ctx.clip();
    ctx.fillStyle='#071522';ctx.fillRect(cx-radius,cy-radius,radius*2,radius*2);
    if(texture.complete&&texture.naturalWidth){const imageWidth=texture.naturalWidth,imageHeight=texture.naturalHeight,rotation=((globeRotation%360)+360)%360/360*imageWidth;for(let y=-radius;y<radius;y+=2){const normalized=y/radius,span=Math.sqrt(Math.max(0,1-normalized*normalized)),sourceWidth=Math.max(2,imageWidth*.5*span),sourceX=(rotation+imageWidth*.5-sourceWidth*.5)%imageWidth,sourceY=Math.floor((normalized*.5+.5)*imageHeight),sourceHeight=Math.min(3,imageHeight-sourceY),destY=cy+y,destHeight=Math.min(2,cy+radius-destY);drawTexture(texture,sourceX,sourceY,sourceWidth,sourceHeight,cx-radius*span,destY,radius*2*span,destHeight)}}
    const shade=ctx.createLinearGradient(cx-radius,cy,cx+radius,cy);shade.addColorStop(0,'rgba(38,203,220,.08)');shade.addColorStop(.57,'rgba(0,0,0,.02)');shade.addColorStop(1,'rgba(3,8,18,.72)');ctx.fillStyle=shade;ctx.fillRect(cx-radius,cy-radius,radius*2,radius*2);ctx.restore();
    const glow=ctx.createRadialGradient(cx-45,cy-55,110,cx,cy,184);glow.addColorStop(.78,'rgba(85,226,239,0)');glow.addColorStop(.94,'rgba(85,226,239,.14)');glow.addColorStop(1,'rgba(187,251,255,.5)');ctx.fillStyle=glow;ctx.beginPath();ctx.arc(cx,cy,radius+3,0,Math.PI*2);ctx.fill();
    globeAnimationFrame=requestAnimationFrame(paint);
  };
  canvas.onpointerdown=event=>{dragging=true;lastX=event.clientX;canvas.setPointerCapture?.(event.pointerId);canvas.classList.add('is-dragging')};
  canvas.onpointermove=event=>{if(!dragging)return;globeRotation+=(event.clientX-lastX)*.32;lastX=event.clientX};
...
function renderDistributionNetwork(){
  ...
  const radians=Math.PI/180,cx=300,cy=198,radius=166,centerLongitude=55+globeRotation,centerLatitude=20*radians;
